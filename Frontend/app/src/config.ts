...
export const API_BASE_CANDIDATES = [API_BASE_URL];
export const API_UPLOAD_BASE =
  API_BASE_URL.replace(/\/api\/?$/, "") || DEFAULT_API_URL.replace(/\/api\/?$/, "");

/** WebSocket URL for support chat (same host as API, ws scheme). */
export const SUPPORT_WS_URL = (() => {
  const base = API_BASE_URL.replace(/\/api\/?$/, "") || DEFAULT_API_URL.replace(/\/api\/?$/, "");
