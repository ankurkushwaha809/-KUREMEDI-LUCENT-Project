...
export default function CategoryByNamePage() {
  const params = useParams();
  const slug = params?.name || "";
  const { getProducts, getCategories, toggleWishlist, isInWishlist, addToCart, token, user } =
    useAppContext();
  ...
  useEffect(() => {
    ...
    getCategories()
      .then((res) => {
        ...
        const cat = list.find((c) => slugify(c?.name || "") === slug);
        if (!cat) {
          setNotFound(true);
          setCategoryName(decodeURIComponent(slug).replace(/-/g, " "));
          setProducts([]);
          setLoading(false);
          return;
        }
        ...
        return getProducts({ category: name });
      })
      .then((res) => {
        if (!res) return;
  ...
  }, [slug, getProducts, getCategories]);
